...
## TODO: Change repo to public
URL_CONFIG_FILE <- "https://raw.githubusercontent.com/DaniMori/outcome-vars/main/src/install_ecs_data.R"
...
URL_CITATION_SYNTAX <- "https://rmarkdown.rstudio.com/authoring_bibliographies_and_citations.html#Citation_Syntax"

URL_BIBTEX_EXPORT_ENDNOTE <- "https://endnote.com/style_download/bibtex-export/"
URL_BIBTEX_EXPORT_MENDELEY <- "https://www.mendeley.com/guides/mendeley-reference-manager/08.-exporting-references"
...
ICON_THUMB_UP     <- "www/assets/thumbs-up.png"
ICON_HAND         <- "www/assets/raised_hand.png"
ICON_PLUS         <- "www/assets/plus.png"

URL_ICON_CONSTRUCTION <- "https://maxcdn.icons8.com/Share/icon/Transport/under_construction1600.png"
...
URL_ICON_EXERCISE_MOM <- "https://img.icons8.com/ios-filled/50/000000/mommy-fitness.png"
URL_ICON_THUMB_UP <- "https://clipartspub.com/search/emoji-clipart-thumbs-up"
URL_ICON_HAND     <- "https://www.clipartmax.com/download/m2i8H7i8Z5Z5m2b1_image-raised-hand-emoji-png/"
...
::: {.notes}
-   Actualización: De los datos si cambia la sintaxis
...
4.  Adquirir habilidades de manejo de Rstudio
:::
...
::: {.notes}
Explicaciones (pocas) con teclas en pantalla

Teclado en pantalla: Comprobar que Carnac esté iniciado

- Es habitual usar el teclado para hacer tareas más sencillas

- Voy a explicar cómo usar tanto el interfaz gráfico como el teclado
(a veces puede que se me olvide y vaya "demasiado rápido",
por eso muestro las teclas)

- Explicar que se mostrarán algunas combinaciones de teclas (atajos)

- Comprobar que se ven (p. ej. `CTRL` + `C`)

- Avisar que puede fallar y dejarse de ver las teclas

Hay muchos ejercicios aplicados

Reacciones: Explicar uso de Polly
...
2.  Comprobar la sincronización de las carpetas de OneDrive localmente
:::
...
::: {.notes}
EXPLICAR: ¿Dónde se encuentra cada una de esas teclas?

Énfasis en distinguir `ALT GR` (derecha de barra espciadora)
y `ALT` (izquierda)

Comentar que `ALT GR` se muestra como `CTRL` + `ALT`
(de hecho puede usarse `CTRL` + `ALT` para escribir ese mismo resultado)

Comprobar que Carnac está activado!
:::
...
-   `ALT` (`CMD`) + `TAB`: Cambiar entre ventanas
...
-   Probar `ALT` + `TAB` (MAC: `CMD` + `TAB`) para cambiar de Teams a R
...
-   Si usas ejemplo "tipo", añade tu nombre como "particularidad"
...
- Delmitador de código, tres acentos inversos

- A continuación las llaves con la *configuración del chunk*
...
> -   Copiar "description" de prototipo
...
Generar outcomes: Sólo para miembros de "Edad con Salud"
...
> -   `CMD` + `TAB`: Cambiar entre ventanas de Windows

> -   `CTRL` + `1`: Ir al editor

> -   `CTRL` + `2`: Ir a la consola

> -   `CMD` + `S`: Guardar archivo / guardar cambios

> -   `CTRL` + `SHIFT` + `F4`: Activar editor visual

> -   `OPTION` + `ENTER`: Ejecutar comando en R

> -   `CMD` + `ENTER`: Ejecutar comando en R y siguiente

> -   `CMD` + `SHIFT` + `K`: "Knitear" documento
...
> -   Relativa = "path" relativo a la ruta actual
